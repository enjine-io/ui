...
function _color(ops = "") {
	if (ops.includes("primary")) return "primary";
	else if (ops.includes("secondary")) return "secondary";
	else return "default";
}
function _variant(ops = "") {
	if (ops.includes("outline")) return "outlined";
	else if (ops.includes("text")) return "text";
	else return "contained";
}
function _size(ops = "") {
	if (ops.includes("small")) return "small";
	else if (ops.includes("large")) return "large";
	else return "medium";
}
function _padding(el, obj, left, top, right, bottom, mode = "") {
	mode = mode.toLowerCase();
	(obj._padding.left = left),
		(obj._padding.top = top),
		(obj._padding.right = right),
		(obj._padding.bottom = bottom),
		(obj._padding.mode = mode);
	if (mode == "v") {
...
function UI() {
	...
	this.libs = window._hybrid
		? app.GetPrivateFolder("Plugins") + "/ui/libs"
		: window._cdn
		? "https://cdn.jsdelivr.net/gh/enjine-io/ui@main"
		: "libs";
	...
	this.setTheme = function (theme) {
		...
		document.getElementById("_id_picker_theme").href =
			self.libs + "/date-time-picker/css" + (self.theme.dark ? "/dark.css" : "/light.css");
		document.getElementById("_id_dtpicker_theme").href =
			self.libs + "/material-datetime-picker-" + (self.theme.dark ? "dark.css" : "light.css");
	...
	this.setFontFile = function (file) {
		...
		const css = "@font-face {" + "font-family: '" + name + "'; " + "src: url('" + file + "'); " + "}";
